...
import { Carousel, CarouselContent, CarouselItem } from "./ui/carausal";
...
const TechStack = () => {
  const plugin = useRef<ReturnType<typeof AutoScroll>>(
    AutoScroll({
      delay: 2000,
  ...
  const skillsData = [
    ...
    {
      icon: "https://somesh9870.github.io/static/media/MaterialUi.5955eca0663eaeb9b8d6.png",
      name: "Material UI",
      category: "frontend",
    },
    {
  ...
  return (
    <div>
      <Carousel plugins={[plugin.current]} opts={{ loop: true }}>
        <div className="absolute inset-y-0 left-0 z-10 w-40 bg-gradient-to-r from-zinc-950"></div>
        <CarouselContent className="-ml-4 items-stretch justify-stretch">
          {skillsData?.map((skill) => (
            <CarouselItem className="relative aspect-square h-full min-h-full grow basis-1/5">
              <div className="">
                <div className=" h-3/4 md:h-1/2  relative max-w-60">
        ...
        <div className="absolute inset-y-0 right-0 z-10 w-40 bg-gradient-to-l from-zinc-950"></div>
      </Carousel>
    </div>
  );
};
